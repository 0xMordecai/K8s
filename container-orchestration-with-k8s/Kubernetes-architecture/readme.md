...
**`KinD`, on the other hand, exclusively runs your nodes as containers on both `single-node` and `multi-node` configurations.**

**You need a VM with the requisite resources in both cases, and you’ll be good to go.**